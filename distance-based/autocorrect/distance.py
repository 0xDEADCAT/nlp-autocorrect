...
class MinimumEdits:
    ...
    _rows: int = None
    _columns: int = None
    _distance_table: np.ndarray = None
    _distance_frame: pd.DataFrame = None
    ...
    def distance_table(self) -> np.ndarray:
        ...
        if self._distance_table is None:
            self._distance_table = np.zeros(
                (self.rows + 1, self.columns + 1), dtype=int
            )
            # initialize the first row
            for row in range(1, self.rows + 1):
                self._distance_table[row, 0] = (
                    self._distance_table[row - 1, 0] + self.deletion_cost
                )
            # initialize the first column
            for column in range(1, self.columns + 1):
                self._distance_table[0, column] = (
                    self._distance_table[0, column - 1] + self.insertion_cost
                )
            ...
            for row in range(1, self.rows + 1):
                ...
                for column in range(1, self.columns + 1):
                    ...
                    replacement_cost = (
                        0
                        if self.source[one_row_back] == self.target[one_column_back]
    ...
    def distance_frame(self) -> pd.DataFrame:
        ...
        if self._distance_frame is None:
            self._distance_frame = pd.DataFrame(
                self.distance_table,
    ...
    def minimum_distance(self) -> int:
        ...
        if self._minimum_distance is None:
            self._minimum_distance = self.distance_table[self.rows, self.columns]
