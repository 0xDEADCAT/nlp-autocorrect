...
from autocorrect.distance import MinimumEdits
...
class Aligner:
        ...
        if self._path is None:
            ...
            while (current_row, current_column) != (0, 0):
                ...
                edits = (
                    # insert
                    (
                        distances[one_row_back, current_column],
                        (one_row_back, current_column),
                    ),
                    # delete
                    (
                        distances[current_row, one_column_up],
                        (current_row, one_column_up),
                    ),
                    # substitute
                    (
                        distances[one_row_back, one_column_up],
                        (one_row_back, one_column_up),
                    ),
                )
    ...
    def __call__(self) -> tuple:
        ...
        for current_row, current_column in self.path[1:]:
            source_token = (
                source[current_row] if current_row != previous_row else self.empty_token
            )
            target_token = (
                target[current_column]
