...
from autocorrect.edits import WordEditor
...
class WordSuggestor:
    ...
    def __call__(self, word: str) -> list:
        ...
        if word in self.corpus.vocabulary:
            ...
            suggestions = self.corpus.vocabulary.intersection(
                self.one_letter_edits(word)
            )
            if not suggestions:
                suggestions = self.corpus.vocabulary.intersection(
                    self.two_letter_edits(word)
                )
            if suggestions:
                probabilities = list(
                    reversed(
                        sorted(
                            [
                                (
                                    self.corpus.probabilities.get(suggestion, 0),
                                    suggestion,
                                )
                ...
                best = [
                    (word, probability)
                    for (probability, word) in probabilities[: self.suggestions]
                ]
            else:
                best = [(word, 0)]
        return best
